...
def get_highest_occurrence(selected_genre_occurrence):
    ...
    if (len(selected_genre_occurrence) > 0):
...
def main(userId, number_of_recommendations):
    # Setup connection
    ...
    movies = pd.read_sql(('SELECT m.movieId AS movieId, genres '
                         'FROM movies m '
                         'INNER JOIN (SELECT movieId, COUNT(*) FROM ratings WHERE rating > 3 AND `userId` != {} '
                         'GROUP BY movieId HAVING COUNT(*) > 0) AS cnt ON cnt.movieId = m.movieId '
                         'WHERE m.Year > YEAR(CURDATE()) - 20').format(str(userId)), con=conn)
...
if __name__ == '__main__':
    ...
    try:
        ...
        data = main(userId, number_of_results)
        print(data)
    except Exception as e:  # most generic exception you can catch
        duc.write(logging.exception("message"))
