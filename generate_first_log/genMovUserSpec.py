...
def get_rand_movies(amount_to_gen, movies, ratings, conn):
    ...
    movies_max_years_old = movies['movieId'][movies['Year'] >= (date.today().year - 10)]

    ratings = ratings[ratings['movieId'].isin(list(movies_max_years_old.values))]
...
def main():
    ...
    if (amount_to_gen > 0):
        # Get movies
        movies = pd.read_sql("SELECT m.movieId as movieId, Year "
        				"FROM movies m left join sampleUserInitial s on s.movieId = m.movieId "
        				"WHERE m.Year IS NOT NULL and s.movieId is null", con=conn)
        
        ratings = pd.read_sql("select * from ratings", con=conn)
        
        if (get_rand_movies(amount_to_gen, movies, ratings, conn)):
            return 'Movies generated'
    else:
        return 'Fifty rows found - No data will be generated'
